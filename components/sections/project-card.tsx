...
export function ProjectCard({ project, delay = 0 }: ProjectCardProps) {
  ...
  const t = uiStrings[lang].projectCard;
    ...
    <Reveal delay={delay} className="group flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/[0.03] transition-colors hover:border-violet-400/30">
      <div className="relative aspect-video w-full overflow-hidden bg-zinc-900">
        {project.isPlaceholder ? (
          <PlaceholderFrame label={t.placeholderImage} className="h-full w-full rounded-none border-0" />
        ) : (
          <Image
            src={image}
            alt={t.screenshotAlt(title[lang])}
            fill
      ...
      <div className="flex flex-1 flex-col gap-4 p-6">
        <div className="flex items-start justify-between gap-3">
          <h3 className="text-lg font-bold text-white">{title[lang]}</h3>
          <Badge variant={statusVariant[status]}>{t.status[status]}</Badge>
        </div>

        <p className="text-sm text-zinc-400">{description[lang]}</p>
        ...
        >
          <div className="flex flex-col gap-3 border-t border-white/10 pt-3 text-xs text-zinc-500">
            <p>
              <span className="font-semibold text-zinc-400">{t.challengesLabel} </span>
              {challenges[lang]}
            </p>
            <p>
              <span className="font-semibold text-zinc-400">{t.learningsLabel} </span>
              {learnings[lang]}
            </p>
          </div>
        </motion.div>
